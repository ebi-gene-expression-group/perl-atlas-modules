#!/usr/bin/env perl
#
# EBI/FGPT/Resource/Database/GXA.pm
#
# $Id: GXA.pm 24026 2013-06-20 13:41:59Z amytang $

package EBI::FGPT::Resource::Database::GXA;
...
sub BUILD{
    ...
    my ($self) = @_;
    $self->set_dsn( $CONFIG->get_AEDW_DSN() );
    $self->set_username ( $CONFIG->get_AEDW_USERNAME() );
    $self->set_password ( $CONFIG->get_AEDW_PASSWORD() );
...
sub fetch_degenes_experiments_contrasts_from_atlasdb {
    ...
    $logger->info( "Querying Atlas database for differentially expressed genes, experiments, and contrast IDs..." );
...
sub fetch_baseline_genes_experiments_assaygroups_from_atlasdb {
    ...
    $logger->info( "Querying Atlas database for baseline genes, experiments, and assay group IDs..." );
...
sub fetch_differential_experiment_info_from_atlasdb {
    ...
    $logger->info( "Querying Atlas database for differential experiment info..." );
...
sub fetch_baseline_experiment_info_from_atlasdb {
    ...
    $logger->info( "Querying Atlas database for baseline experiment info..." );
...
sub fetch_isl_processing_experiments {
    ...
    my $query = "
        ...
        where jobtype in ( 'isl_public', 'isl_private', 'baseline/rna-seq', 'differential/rna-seq' )";
